...
\end{columns}
   ...
   \begin{itemize}
    \item \noindent Two groups: 1.: pitch $p \sim 10\, \mathrm{nm}$ (bent-core LC), \\ 2.: pitch in optical range (rod-like \& linked by flexible chain)
    \end{itemize}
    \begin{columns}[c]
...
\begin{slide}{Contents}
 \begin{itemize}
  \item Determining the position and width of heliconical band gap with changing material properties ($\theta$, $p$)
  \item Description of electric $\vec{E}$ and magnetic $\vec{H}$ fields inside heliconics: $\vec{E}$, $\vec{H}$-eigenmodes
  \item Winding of the Poynting vector $\vec{P}$ 
  \vspace{0.8cm}
  \item Numerical methods used: 
    \begin{itemize}
      \item[--] Frequency domain method: eigenvalue method for determining the band diagrams and eigenmodes
      \item[--] FDTD: Finite-difference time-domain method: solving time-dependent Maxwell's equation, gives info about light propagation
    \end{itemize}  
  \end{itemize}
\end{slide}
...
\begin{slide}{Opening of the BG with $\theta$}
  ...
  \vspace{10pt}
    We have quantified the band-gap position and width
    \vspace{10pt}
  \end{textblock*}}
\end{slide}
...
We have determined the photonic properties of heliconical liquid crystals: 
\begin{itemize}
